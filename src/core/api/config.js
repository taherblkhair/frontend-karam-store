 ...
 *
 * Production build with direct API:
 *   VITE_API_BASE_URL=https://api.karamstore.ly
 *   → browser calls API host directly (server must send CORS headers)
 */
...
/**
 * Media origin for /uploads paths.
 * When using proxy mode, leave empty so images hit /uploads on the same host (Vite proxies them).
 * When direct API, rewrite to API host.
 */

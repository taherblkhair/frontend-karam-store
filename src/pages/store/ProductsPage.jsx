...
import { ProductCard, LoadingSpinner, EmptyState } from '../../components/ui';
...
export default function ProductsPage() {
  ...
  const { data: productsData, isLoading } = useQuery({
    ...
    queryFn: () => api.get('/store/products', { params: filters }),
  });

  const { data: categoriesData } = useQuery({
    queryKey: ['categories'],
    queryFn: () => api.get('/store/categories'),
  });

  const { data: colorsData } = useQuery({
    queryKey: ['colors'],
    queryFn: () => api.get('/store/colors'),
  });

  const { data: sizesData } = useQuery({
    queryKey: ['sizes'],
    queryFn: () => api.get('/store/sizes'),
  });
